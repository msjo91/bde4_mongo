...
from .forms import SigninForm, SignupForm, ChangeProfileImageModelForm, SignupModelForm
...
def signin_fbv(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            # 전달되어온 POST데이터에서 'email'과 'password'키의 값들을 사용
            email = form.cleaned_data['email']
            password = form.cleaned_data['password']
            # authenticate의 인자로 POST로 전달받은 email, password를 사용
            user = authenticate(email=email, password=password)

            # 만약 인증이 정상적으로 완료되었다면
            # (해당하는 email, password에 일치하는 User객체가 존재할경우)
            if user is not None:
                # Django의 인증관리 시스템을 이용하여 세션을 관리해주기 위해 login()함수 사용
                login(request, user)
                return redirect('post:list')
            # 인증에 실패하였다면 (email, password에 일치하는 User객체가 존재하지 않을 경우)
            else:
                form.add_error(None, 'Email or PW incorrect')
